...
from src.config import (TARGET_ACCOUNT as DEFAULT_ACCOUNT,
...
def scheduled_job():
    ...
    new_posts = [p for p in posts if p["id"] not in existing_ids and p["media_type"] == "video"]
...
def start():
    ...
    if scheduler.running:
        ...
        scheduler.add_job(scheduled_job, "interval", minutes=data["interval"], id="scraper_job")
        scheduler.start()
